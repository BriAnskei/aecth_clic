...
namespace aesth_clic.Views.Roles.Admin.Pages
{
    ...
    public sealed partial class UserManagement : Page
    {
        ...
        private void LoadSampleData()
        {
            ...
            {
                BuildItem("u1", "Dr. Maria Santos",  "maria@clinic.com",  "0912-345-6789", "Doctor",       "Active"),
                BuildItem("u2", "Jose Reyes",         "jose@clinic.com",   "0923-456-7890", "Receptionist", "Active"),
                BuildItem("u3", "Ana Cruz",           "ana@clinic.com",    "0934-567-8901", "Pharmacist",   "Active"),
                BuildItem("u4", "Dr. Carlo Mendoza",  "carlo@clinic.com",  "0945-678-9012", "Doctor",       "Inactive"),
                BuildItem("u5", "Liza Flores",        "liza@clinic.com",   "0956-789-0123", "Receptionist", "Inactive"),
            };
        }
        ...
        {
            if (UserListControl is null) return; // ← safety guard
            ...
            if (TxtTotalUsers is not null) TxtTotalUsers.Text = _allUsers.Count.ToString();
            if (TxtActiveUsers is not null) TxtActiveUsers.Text = _allUsers.Count(u => u.Status == "Active").ToString();
            if (TxtInactiveUsers is not null) TxtInactiveUsers.Text = _allUsers.Count(u => u.Status == "Inactive").ToString();
            if (TxtRowCount is not null) TxtRowCount.Text = $"Showing {filtered.Count} user{(filtered.Count == 1 ? "" : "s")}";
        }
        ...
        {
            // TODO: replace with your real Add User dialog / navigation
            var dialog = new ContentDialog
            {
                Title = "Add User",
                Content = "Add User dialog goes here.",
        ...
        private async void ReactivateUser_Click(object sender, RoutedEventArgs e)
        {
            ...
            var confirm = new ContentDialog
            {
                ...
                Content = $"Reactivate {user.FullName}?",
